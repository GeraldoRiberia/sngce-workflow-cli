...
Usage:
    ...
    python main.py --backend        # backend health + SSL only
    python main.py --frontend       # frontend sync + SSL only
...
def run_checks(config: dict, backend_only: bool = False, frontend_only: bool = False):
    ...
    for key, server in servers.items():
        ...
        # --- Backend check ---
        if not frontend_only and "backend_url" in server:
            ...
            ssl_result = check_ssl(server["backend_url"])
            display.print_ssl_result(label, ssl_result)

        # --- Frontend sync check ---
        if not backend_only and "frontend_url" in server:
...
def main():
    ...
    if args.watch:
        ...
        try:
            while True:
                run_checks(config, backend_only=args.backend, frontend_only=args.frontend)
